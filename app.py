...
# Keeping this override as it resolves the SQLite version issue required by Chroma,
# even if persistence isn't the primary goal anymore. It ensures Chroma's internal
# operations use a compatible SQLite version.
try:
    print("Attempting to override sqlite3 with pysqlite3...")
    __import__('pysqlite3')
    import sys
    # Check if 'sqlite3' is already the pysqlite3 module to avoid recursion error
    if 'sqlite3' not in sys.modules or sys.modules['sqlite3'].__name__ != 'pysqlite3.dbapi2':
        sqlite3_original = sys.modules.get('sqlite3') # Store original if exists
        sys.modules['sqlite3'] = sys.modules.pop('pysqlite3')
        print("Successfully replaced sqlite3 with pysqlite3.")
    else:
        print("sqlite3 already appears to be pysqlite3.")
except ImportError:
...
def load_and_process_pdfs(pdf_files, chunk_size=1500, chunk_overlap=300):
    """Loads PDFs, splits them, filters metadata, and creates an IN-MEMORY vector store."""
    all_docs = []
    ...
    # Filter metadata (still useful even if less complex from PyPDFLoader)
    st.info(f"Loaded {len(all_docs)} pages/sections. Filtering complex metadata...")
    try:
        # Ensure filter_complex_metadata handles empty list gracefully if needed, though we check above
        if all_docs:
    ...
    try:
        ...
        vector_db = Chroma.from_documents(
            ...
            # collection_name="langchain_in_memory" # Optional name, defaults usually ok
        )
...
def main():
    ...
    with st.sidebar:
        ...
        st.warning("Vector store is In-Memory. PDFs must be re-processed if app restarts or session ends.")

        uploaded_files = st.file_uploader(
            "Upload PDF files", type="pdf", accept_multiple_files=True, key="pdf_uploader_inmemory_v2"
        )

        # Simplified button logic - always processes uploaded files into memory
        if st.button(f"Process Uploaded PDF(s)", key="process_button_inmemory_v2", disabled=not uploaded_files):
            with st.spinner(f"Processing PDFs into memory..."):

                # --- Simplified Processing Steps for In-Memory ---
                # 1. Always start fresh for session state when button is clicked
                st.session_state.vector_db = None # Clear potentially old in-memory db object from session state
                st.session_state.qa_chain = None
                st.session_state.messages = [] # Reset chat history
                st.session_state.processed_files_info = None # Reset file info

                # 2. Process PDFs and create a new IN-MEMORY vector store
        ...
        if st.button("Clear Chat History", key="clear_chat_inmemory_v2"):
            st.session_state.messages = []
            st.rerun()

    st.header("💬 Chat with Your Documents")
    # (Chat display and input logic remains the same)
